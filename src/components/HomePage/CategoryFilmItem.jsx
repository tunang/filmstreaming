...
const CategoryFilmItem = ({title, api, action}) => {
    ...
    return (<>
        ...
        <div className="flex h-[220px] md:h-[600px] gap-5 mt-8 loading overflow-x-auto scrollbar scrollbar-thumb-tertiary scrollbar-track-transparent">
            {films && films.data && films.data.data && films.data.data.items.map((item, index) => {
                return <div onClick={() => handleClick(item._id, item.slug)} className="relative min-w-[40%] md:min-w-[25%] pb-10 md:pb-20 bg-quaternary rounded overflow-hidden "> 
                    ...
                    <p className="absolute text-[12px] text-black top-[15%] left-[5%] w-fit px-2 rounded bg-quinary">{item.quality}</p>
                </div>
            })}
